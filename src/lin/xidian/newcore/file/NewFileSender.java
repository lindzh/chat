...
public class NewFileSender extends TransferSenderBase implements Runnable
{
		...
		if(file.exists())
		{
			try {
				...
				while(isSending&&hasSended<fileLength)
				{
					 ...
					 if(rst!=-1)
					 {
						 byte[] data = NetFileUtils.toBytes(id, 0, position, rst, buffer);
						 ...
						 position += rst;
						 System.out.println("SendLen:"+hasSended+"  fileLen:"+fileLength);
					 }
				}
				fis.close();
				if(isSending)//正常结束
