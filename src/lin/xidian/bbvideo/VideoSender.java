...
public class VideoSender extends VideoSenderBase implements ControllerListener,Runnable
{
	...
	private DataSource audioOutput;
	...
	public void run()
	{
		...
		{
			audioLocator = ((CaptureDeviceInfo)audios.get(0)).getLocator();
			audioProcessor = createProcessor(audioLocator);
			if(audioProcessor != null)
			{
				audioDataSource = mediaData;
				audioOutput = audioProcessor.getDataOutput();
			}
		}
		...
		dataSources[0] = audioDataSource;
		dataSources[1] = videoDataSource;
		try {
			//DataSource dataLocal = Manager.createMergingDataSource(dataSources);
		...
		dataSources[0] = audioOutput;
		dataSources[1] = videoOutput;
		try {
			DataSource dataOutput = Manager.createMergingDataSource(dataSources);
			createTransmitter(dataOutput);
		...
		if(pbs.length == 2)
		{
			for(int i=0;i<2;i++)
			{
				try
				{
					...
					SendStream sendStream = managers[i].createSendStream(dataOpt, i);
					sendStream.start();
				}catch(Exception e)
				{
					startOK = false;
